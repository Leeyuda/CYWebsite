...
function initializeAllocationChart() {
    ...
    // Data for the pie chart - 從HTML讀取顏色
    const data = [
        { label: '教育服務發展與執行', value: 51, color: getColorFromDot(0) },
        { label: '服務地區與總部國際團隊發展', value: 19, color: getColorFromDot(1) },
        { label: '影響力推廣與募款', value: 17, color: getColorFromDot(2) },
        { label: '總部支持系統', value: 13, color: getColorFromDot(3) }
    ];
    ...
    function animateChart() {
        ...
        if (animationProgress >= 1) {
            ...
            data.forEach((segment, index) => {
                ...
                // Draw label text (shortened version for better fit)
                const shortLabels = {
                    '教育服務發展與執行': '教育服務發展',
                    '服務地區與總部國際團隊發展': '團隊發展',
                    '影響力推廣與募款': '推廣募款',
                ...
                ctx.font = `bold ${Math.max(12, size * 0.045 + 2)}px ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif`; // 響應式字體大小，增加2px，使用Tailwind預設字型
                ...
                ctx.shadowOffsetX = 0;
                ctx.shadowOffsetY = 0;
                ctx.fillStyle = '#f3f4f6'; // very light gray, closer to white
                ctx.fillText(shortLabels[segment.label] || segment.label, textX, textY + 12);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function animateProgressBars() {
        ...
        // 更新目標金額顯示
        targetAmountEl.textContent = 'NT$' + targetAmount.toLocaleString();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 樓層地圖圖片配置（您可以在這裡替換為實際的樓層地圖圖片）
    const floorMapImages = {
        'B3': 'https://www.cybaby.org.tw/upload/images/20251216101944.jpg', // 替換為 B3 地圖
        'B2': 'https://www.cybaby.org.tw/upload/images/20251216101944.jpg', // 替換為 B2 地圖
        'B1': 'https://www.cybaby.org.tw/upload/images/20251216101944.jpg', // 替換為 B1 地圖
        '1F': 'https://www.cybaby.org.tw/upload/images/20251216101944.jpg'  // 1F 地圖
    };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateMilestoneStyles(currentStage) {
        ...
        milestoneItems.forEach(item => {
            ...
            if (stageNum < currentStageNum) {
                ...
                icon.innerHTML = '<i class="ri-check-line text-sm md:text-base"></i>';
                badge.classList.add('bg-green-100', 'text-green-800');
            } else if (stageNum === currentStageNum) {
                // 正進行：時鐘圖示 + 橘色背景
                icon.classList.add('bg-amber-500', 'text-white');
                icon.innerHTML = '<i class="ri-time-line text-sm md:text-base"></i>';
                badge.classList.add('bg-amber-100', 'text-amber-800');
            } else {
                // 未進行：日曆圖示 + 灰色背景
                icon.classList.add('bg-gray-300', 'text-gray-600');
                icon.innerHTML = '<i class="ri-calendar-line text-sm md:text-base"></i>';
    ...
    function initMilestoneProgress() {
        const currentStage = currentProgressEl.textContent.trim() || currentProgressEl.getAttribute('data-progress') || '3';
    ...
    function setupProgressListener() {
        ...
        currentProgressEl.addEventListener('input', function() {
            ...
            if (newStage && !isNaN(newStage) && newStage >= 1 && newStage <= 6) {
                ...
                // 如果輸入無效，恢復原值
                const validStage = this.getAttribute('data-progress') || '3';

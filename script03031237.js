...
// 募資進度條（與 HTML 的 #progress-bar、#raised-amount、#target-amount 等一致，先執行以配合載入順序）
function animateFundraisingProgress() {
    var progressBar = document.getElementById('progress-bar');
    var raisedAmountEl = document.getElementById('raised-amount');
    var targetAmountEl = document.getElementById('target-amount');
    if (!progressBar || !raisedAmountEl || !targetAmountEl) return;

    var raisedAmount = parseInt(raisedAmountEl.getAttribute('data-amount'), 10) || parseInt(raisedAmountEl.textContent.replace(/[^0-9]/g, ''), 10) || 0;
    var targetAmount = parseInt(targetAmountEl.getAttribute('data-target'), 10) || parseInt(targetAmountEl.textContent.replace(/[^0-9]/g, ''), 10) || 0;
    var percentage = targetAmount > 0 ? (raisedAmount / targetAmount * 100) : 0;
    ...
    var achievementRateEl = document.getElementById('achievement-rate');
    var remainingAmountEl = document.getElementById('remaining-amount');
    var supportersCountEl = document.getElementById('supporters-count');
    if (achievementRateEl) achievementRateEl.textContent = (targetAmount > 0 ? (raisedAmount / targetAmount * 100) : 0).toFixed(1) + '%';
    if (remainingAmountEl) remainingAmountEl.textContent = 'NT$' + Math.max(0, targetAmount - raisedAmount).toLocaleString();
    if (supportersCountEl) {
        var count = parseInt(supportersCountEl.getAttribute('data-count'), 10) || parseInt(supportersCountEl.textContent.replace(/[^0-9]/g, ''), 10) || 0;
        var hasPlus = supportersCountEl.textContent.indexOf('+') !== -1;
        supportersCountEl.textContent = count.toLocaleString() + (hasPlus ? '+' : '');
    }
    raisedAmountEl.textContent = 'NT$' + raisedAmount.toLocaleString();
}
...
// Intersection Observer for progress animation（animateFundraisingProgress 使用 #progress-bar，與 HTML 一致）
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 載入順序或後台環境可能讓 observer 不觸發，延遲再執行一次募資進度條
    setTimeout(animateFundraisingProgress, 500);
});

// 全資源載入後再跑一次，確保後台/CMS 動態內容後也能顯示進度條
window.addEventListener('load', function () {
    var bar = document.getElementById('progress-bar');
    if (bar && parseFloat(bar.style.width) === 0) animateFundraisingProgress();
});
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 當進度區塊進入視窗時觸發動畫；後台/iframe 等環境可能不觸發，故加延遲備援
    ...
    if (progressSection) {
        ...
        // 若 1 秒後仍未觸發（例如區塊已在視窗內或後台環境），仍執行一次
        setTimeout(function () {
